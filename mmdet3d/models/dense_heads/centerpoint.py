...
from mmdet3d.ops.iou3d.iou3d_utils import nms_gpu, multi_class_nms_gpu
...
class CenterHead(BaseModule):
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            heatmaps.append(heatmap.to(gt_bboxes_3d.device))
    ...
    @force_fp32()
    def loss_iou(self, pred, pred_iou, target_bbox, ind, mask):
        """IoULoss
        Args:
            pred ([torch.Tensor]): Size B x N_obj x 10
            pred_iou ([torch.Tensor]): size B x N_obj
            target_bbox ([torch.Tensor]): Size B x N_obj x 10
            ind ([torch.Tensor]): size B x N_obj
            mask ([torch.Tensor]): size B x N_obj
        Returns:
            loss_iou: scalar (FP32)
        """
        grid_size = torch.tensor(self.train_cfg["grid_size"])
        pc_range = torch.tensor(self.train_cfg["point_cloud_range"])
        voxel_size = torch.tensor(self.train_cfg["voxel_size"])
        # convert predictions & GT to lidar coordinate system
        with torch.no_grad():
            converted_boxes = []
            for orig in [pred, target_bbox]:
                new_pred = torch.zeros_like(orig[..., :7])
                # x and y
                new_pred[..., 0] = (
                    orig[..., 0] + (ind // grid_size[1]).float()
                ) * self.train_cfg["out_size_factor"] * voxel_size[0] + pc_range[0]
                new_pred[..., 1] = (
                    orig[..., 1] + (ind % grid_size[1]).float()
                ) * self.train_cfg["out_size_factor"] * voxel_size[1] + pc_range[1]
                # z
                new_pred[..., 2] = orig[..., 2]
                # box size
                if self.norm_bbox:
                    new_pred[..., 3:6] = torch.exp(orig[..., 3:6])
                else:
                    new_pred[..., 3:6] = orig[..., 3:6]
                # heading angle
                new_pred[..., 6] = torch.atan2(orig[..., 6], orig[..., 7])
                converted_boxes.append(new_pred)

            # calculate IoU
            predicted_boxes, gt_boxes = converted_boxes[0], converted_boxes[1]
            predicted_boxes = predicted_boxes.reshape(-1, 7)
            gt_boxes = gt_boxes.reshape(-1, 7)
            gt_iou = bbox_overlaps_3d(
                predicted_boxes, gt_boxes, mode="iou_correspond", coordinate="lidar"
            ).reshape(mask.shape)
        loss_iou = torch.sum(torch.abs(pred_iou - gt_iou) * mask) / (1e-4 + mask.sum())
        return loss_iou
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            # used for iou loss
            h, w = preds_dict[0]["heatmap"].shape[-2:]
            ...
            if 'iou' in preds_dict[0]:
                pred_iou = preds_dict[0]['iou'].permute(0, 2, 3, 1).contiguous()
                pred_iou = pred_iou.view(pred_iou.size(0), -1, pred_iou.size(3))
                pred_iou = self._gather_feat(pred_iou, ind).squeeze(-1)
                loss_iou = self.loss_iou(pred, pred_iou, target_box, ind, mask[..., 0])
                loss_iou *= self.train_cfg.get('iou_weight', 1.0)
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if "iou" in preds_dict[0]:
                batch_iou = torch.clamp(preds_dict[0]["iou"], 0, 1)
                iou_pow = self.test_cfg.get("iou_pow", 2.0)
                if not isinstance(iou_pow, list):
                    batch_heatmap = batch_heatmap * torch.pow(batch_iou, iou_pow)
                else:
                    for cls_id in range(len(iou_pow)):
                        batch_heatmap[:, cls_id] = torch.pow(
                            batch_heatmap[:, cls_id], 1 - iou_pow[cls_id]
                        ) * torch.pow(batch_iou, iou_pow[cls_id])
            ...
            if top_scores.shape[0] != 0:
                ...
                if not isinstance(self.test_cfg["nms_thr"], list):
                    ...
                    (
                        ...
                        top_scores,
                        top_labels,
                        threshs=self.test_cfg["nms_thr"],
                        pre_max_sizes=self.test_cfg["pre_max_size"],
                        post_max_sizes=self.test_cfg["post_max_size"],
                    )
            else:
                selected = []
